...
def breadcrumb_jsonld(crumbs, h1: str) -> str:
    """현재 페이지 경로로 BreadcrumbList 구조화 데이터를 만든다."""
    base = BASE_URL.rstrip("/")
    items = [("홈", base + HOME)]
    for label, href in crumbs:
        url = (base + href) if href else ""
        items.append((label, url))
    # 마지막 항목(현재 페이지)은 h1 으로 라벨 보정, url 없는 경우 그대로
    elems = []
    for i, (label, url) in enumerate(items, start=1):
        item = f'"name": "{label}"'
        if url:
            item += f', "item": "{url}"'
        elems.append(f'{{ "@type": "ListItem", "position": {i}, {item} }}')
    body = ",\n    ".join(elems)
    return (
...
def page_jsonld(title: str, desc: str, canonical: str) -> str:
    """모든 페이지 공통 — WebPage + Organization + ImageObject(선호 이미지) 구조화 데이터."""
    base = BASE_URL.rstrip("/")
...
def render_page(page: dict) -> str:
    ...
    # 공통 구조화 데이터(WebPage·Organization·ImageObject) + 경로 BreadcrumbList
    schema_head = page_jsonld(title, desc, canonical)
    if crumbs:
        schema_head += breadcrumb_jsonld(crumbs, h1)
...
<header class="site-header">
  ...
  <div class="header-top">
    <div class="header-inner">
      ...
      <p class="header-tagline"><span class="tag-gem">◆</span> 시흥시 전지역 방문 관리 <span class="tag-gem">◆</span> 24시간 상담</p>
...
<footer class="site-footer">
  <div class="container footer-grid">
    <div class="footer-col footer-about">
      ...
      <p class="footer-desc">시흥시 전지역 방문 출장마사지·홈타이 안내 사이트입니다. 모든 서비스는 안내된 관리 범위와 위생·안전 기준 안에서만 제공됩니다.</p>
      <address class="footer-contact">
        ...
        <span class="footer-contact-row"><span class="footer-label">서비스 지역</span> 경기도 시흥시 전지역</span>
      </address>
      <div class="footer-inquiry">
        <a class="inquiry-btn" href="{TELEGRAM_BUILD}" target="_blank" rel="noopener nofollow">웹사이트 제작문의 <span aria-hidden="true">↗</span></a>
        <a class="inquiry-btn" href="{TELEGRAM_PARTNER}" target="_blank" rel="noopener nofollow">제휴문의 <span aria-hidden="true">↗</span></a>
      </div>
    </div>
